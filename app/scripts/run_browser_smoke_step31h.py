"""Capture full visible prose from the browser shell for Step 31H.

Same harness as Step 31G; artifacts are written to `browser_smoke_step31H.*` by default
for before/after comparison. This step focuses on share-layer grammar/context repair.
Does not change engine behavior, retrieval scoring, or public schema.

Usage:
    PYTHONPATH=. python -m app.scripts.run_browser_smoke_step31h
"""
...
def main() -> int:
    ...
    parser.add_argument("--json-out", default="artifacts/browser_smoke_step31H.json")
    parser.add_argument("--md-out", default="artifacts/browser_smoke_step31H.md")
...
def _run_browser_capture(base_url: str) -> dict[str, Any]:
    ...
    return {
        "step": "31H",
        "title": "Browser smoke full visible prose capture (share grammar/context repair)",
        ...
        "warnings": warnings,
        "cases": results,
        "note": "Step 31H run after share-layer presentation rewrite. No retrieval/engine/schema changes; presentation-only copy update.",
    }
...
def _markdown_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Step 31H Browser Smoke Full Prose Capture (share grammar/context repair)",
        "",
        "This artifact captures visible browser prose after share-layer repair; compare with Step 31D/31F/31G for before/after.",
        "",
        "- Before/after note: closest-lens honesty remains, while share quote/question grammar and context matching are repaired in presentation only.",
        "",
        "## Summary",
        "",
