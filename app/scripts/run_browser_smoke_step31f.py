"""Capture full visible prose from the browser shell for Step 31F (post crisis-safe mode).

Same harness as Step 31D; artifacts are written to `browser_smoke_step31F.*` by default
for before/after comparison. Does not change engine behavior, retrieval, or public schema.

Usage:
    PYTHONPATH=. python -m app.scripts.run_browser_smoke_step31f
"""
...
def main() -> int:
    ...
    parser.add_argument("--json-out", default="artifacts/browser_smoke_step31F.json")
    parser.add_argument("--md-out", default="artifacts/browser_smoke_step31F.md")
...
def _run_browser_capture(base_url: str) -> dict[str, Any]:
    ...
    return {
        "step": "31F",
        "title": "Browser smoke full visible prose capture (post crisis-safe presentation)",
        ...
        "warnings": warnings,
        "cases": results,
        "note": "Step 31F run after crisis_safe presentation mode. No engine/schema changes; UI-only safety layer.",
    }
