"""Capture full visible prose from the browser shell for Step 31I.

Same harness as Step 31H; artifacts are written to `browser_smoke_step31I.*` by default
for before/after comparison. This step focuses on counterfactual presentation rewrite.
Does not change engine behavior, retrieval scoring, or public schema.

Usage:
    PYTHONPATH=. python -m app.scripts.run_browser_smoke_step31i
"""
...
def main() -> int:
    ...
    parser.add_argument("--json-out", default="artifacts/browser_smoke_step31I.json")
    parser.add_argument("--md-out", default="artifacts/browser_smoke_step31I.md")
...
def _run_browser_capture(base_url: str) -> dict[str, Any]:
    ...
    return {
        "step": "31I",
        "title": "Browser smoke full visible prose capture (counterfactual rewrite)",
        ...
        "warnings": warnings,
        "cases": results,
        "note": "Step 31I run after counterfactual presentation rewrite. No retrieval/engine/schema changes; presentation-only copy update.",
    }
...
def _markdown_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Step 31I Browser Smoke Full Prose Capture (counterfactual rewrite)",
        "",
        "This artifact captures visible browser prose after counterfactual rewrite; compare with Step 31D/31F/31H for before/after.",
        "",
        "- Before/after note: counterfactual paths are now presentation-layer domain templates; raw engine generation is unchanged.",
        "",
        "## Summary",
        "",
