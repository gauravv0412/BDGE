"""Capture full visible prose from the browser shell for Step 31L.

Same harness as Step 31K; artifacts are written to `browser_smoke_step31L.*` by default
for before/after comparison. This step focuses on domain-detection hardening checkpoint.
Does not change engine behavior, retrieval scoring, or public schema.

Usage:
    PYTHONPATH=. python -m app.scripts.run_browser_smoke_step31l
"""
...
def main() -> int:
    ...
    parser.add_argument("--json-out", default="artifacts/browser_smoke_step31L.json")
    parser.add_argument("--md-out", default="artifacts/browser_smoke_step31L.md")
...
def _run_browser_capture(base_url: str) -> dict[str, Any]:
    ...
    return {
        "step": "31L",
        "title": "Browser smoke full visible prose capture (domain hardening checkpoint)",
        ...
        "warnings": warnings,
        "cases": results,
        "note": "Step 31L run after dilemma-first domain detection hardening. No retrieval/engine/schema changes; presentation-only copy update.",
    }
...
def _build_warnings(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    guidance_primary_debug_word_leakage: list[str] = []
    guidance_explain_debug_word_leakage: list[str] = []
    verdict_fallback_engine_word_leakage: list[str] = []
    domain_mismatch_warnings: list[str] = []
    debug_terms = ("engine", "threshold", "fallback", "verse_match", "retrieval", "schema", "signals", "signal", "dominant ethical pull", "theme", "theme tags", "applies", "metadata", "classifier")
    for item in results:
        cards = item["cards"]
        ...
        expected_domain: dict[str, str] = {
            "wallet_found_cash": "integrity_property",
            "manager_takes_credit_public_correction": "workplace_public_correction",
            "legal_alcohol_shop": "livelihood_community_harm",
            "friends_partner_desire": "desire_betrayal",
            "aging_parent_refuses_hospitalization": "family_medical_autonomy",
            "cosmetic_surgery": "body_insecurity",
            "abusive_parent_no_contact": "abuse_boundary",
            "caste_disapproved_marriage": "caste_equality",
            "anonymous_scathing_restaurant_review": "public_review_retaliation",
            "doctor_hiding_terminal_diagnosis": "medical_truth_consent",
            "low_information_vague_input": "low_information",
        }.get(case_id)
        if expected_domain and domain != expected_domain:
            domain_mismatch_warnings.append(f"{case_id}:expected={expected_domain}:actual={domain}")
        for card in cards:
            ...
            if card.get("card") == "guidance":
                p_low = primary.lower()
                if any(term in p_low for term in debug_terms):
                    guidance_primary_debug_word_leakage.append(f"{item['case_id']}:{primary[:140]}")
            ...
            if card.get("card") == "verdict":
                for section in card.get("sections", []):
                    if section.get("label") == "Explain simply":
                        txt = str(section.get("text", "")).lower()
                        if "engine" in txt:
                            verdict_fallback_engine_word_leakage.append(f"{item['case_id']}:{str(section.get('text', ''))[:140]}")
            if card.get("card") == "guidance":
                for section in card.get("sections", []):
                    if section.get("label") == "Explain simply":
                        txt = str(section.get("text", "")).lower()
                        if any(term in txt for term in debug_terms):
                            guidance_explain_debug_word_leakage.append(f"{item['case_id']}:{str(section.get('text', ''))[:140]}")
    ...
    return {
        ...
        "guidance_primary_debug_word_leakage": guidance_primary_debug_word_leakage,
        "guidance_explain_debug_word_leakage": guidance_explain_debug_word_leakage,
        "verdict_fallback_engine_word_leakage": verdict_fallback_engine_word_leakage,
        "domain_mismatch_warnings": domain_mismatch_warnings,
        "crisis_suppression_status": "ok" if not crisis_counterfactual_not_suppressed else "failed",
    }
...
def _markdown_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Step 31L Browser Smoke Full Prose Capture (domain hardening checkpoint)",
        "",
        "This artifact captures visible browser prose after dilemma-first presentation domain detection hardening.",
        "",
        "- Before/after note (31D -> 31L): crisis-safe suppression, closest-lens rewrite, share/counterfactual/higher-path improvements, sanitization hardening, and domain-misfire reduction are in place.",
        "",
        "## Summary",
        "",
        ...
        f"- Guidance primary debug-word leakage count: {len(warnings.get('guidance_primary_debug_word_leakage', []))}",
        f"- Guidance explain debug-word leakage count: {len(warnings.get('guidance_explain_debug_word_leakage', []))}",
        f"- Verdict fallback engine-word leakage count: {len(warnings.get('verdict_fallback_engine_word_leakage', []))}",
        f"- Domain mismatch warnings: {len(warnings.get('domain_mismatch_warnings', []))}",
        f"- Crisis suppression status: {warnings.get('crisis_suppression_status', 'unknown')}",
        "",
    ]
    ...
    if warnings.get("guidance_metadata_word_leakage"):
        ...
        lines.append("")
    if warnings.get("guidance_primary_debug_word_leakage"):
        lines.extend(["### Guidance Primary Debug-word Leakage", ""])
        lines.extend(f"- `{item}`" for item in warnings["guidance_primary_debug_word_leakage"][:40])
        lines.append("")
    if warnings.get("guidance_explain_debug_word_leakage"):
        lines.extend(["### Guidance Explain Debug-word Leakage", ""])
        lines.extend(f"- `{item}`" for item in warnings["guidance_explain_debug_word_leakage"][:40])
        lines.append("")
    if warnings.get("verdict_fallback_engine_word_leakage"):
        lines.extend(["### Verdict Fallback Engine-word Leakage", ""])
        lines.extend(f"- `{item}`" for item in warnings["verdict_fallback_engine_word_leakage"][:40])
        lines.append("")
    if warnings.get("domain_mismatch_warnings"):
        lines.extend(["### Domain Mismatch Warnings", ""])
        lines.extend(f"- `{item}`" for item in warnings["domain_mismatch_warnings"][:40])
        lines.append("")
