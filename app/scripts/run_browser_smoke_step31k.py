"""Capture full visible prose from the browser shell for Step 31K.

Same harness as Step 31J; artifacts are written to `browser_smoke_step31K.*` by default
for before/after comparison. This step focuses on guidance primary sanitization hardening.
Does not change engine behavior, retrieval scoring, or public schema.

Usage:
    PYTHONPATH=. python -m app.scripts.run_browser_smoke_step31k
"""
...
def main() -> int:
    ...
    parser.add_argument("--json-out", default="artifacts/browser_smoke_step31K.json")
    parser.add_argument("--md-out", default="artifacts/browser_smoke_step31K.md")
...
def _run_browser_capture(base_url: str) -> dict[str, Any]:
    ...
    return {
        "step": "31K",
        "title": "Browser smoke full visible prose capture (guidance sanitization hardening)",
        ...
        "warnings": warnings,
        "cases": results,
        "note": "Step 31K run after guidance primary sanitization and verdict fallback cleanup. No retrieval/engine/schema changes; presentation-only copy update.",
    }
